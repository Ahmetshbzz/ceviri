...
struct ModernTextEditor: View {
    ...
    private var backgroundColor: Color {
        colorScheme == .dark ? Color(UIColor.systemGray6) : Color.clear
    }
        ...
        VStack(spacing: 0) {
            ...
            ZStack(alignment: .topLeading) {
                ...
                UIKitTextEditor(text: $text, isEditable: isEditable, onCommit: onCommit)
                    ...
                    .background( // This will be the fill (clear in light mode)
                        RoundedRectangle(cornerRadius: 10)
                            .fill(backgroundColor)
                            .shadow(color: colorScheme == .dark ? Color.black.opacity(0.03) : Color.clear, radius: 2, x: 0, y: 1) // Conditional shadow
                    )
                    .onTapGesture {
                        if isEditable {
                            isFocused = true
